...
static const int8_t nl_zc_table16[30][12] = {
...
static int nl_math_is_prime(uint16_t n)
{
    uint16_t d;
    if (n < 2u) {
        return 0;
...
int nl_zc_gen_m2_table(uint8_t u, float alpha, nl_cplx_t *seq, uint16_t M_rs)
{
    ...
    for (n = 0u; n < M_rs; n++) {
        int8_t  phi = nl_zc_table16[u][n % 12u];
...
static int nl_zc_gen_zc(uint8_t u, uint8_t v, float alpha, uint16_t M_rs,
                        nl_cplx_t *seq)
{
    uint16_t N_zc;
    float    q_bar;
    ...
    N_zc = nl_zc_largest_prime_less(M_rs);
    if (N_zc == 0u) {
        return NL_PHY_ERR_PARAM;
    }
              ...
              uint16_t M_rs)
{
    ...
    if (m == 2u) {
        if (v != 0u) {
            return NL_PHY_ERR_MODE;
        }
        return nl_zc_gen_m2_table(u, alpha, seq, M_rs);
    }
    if (m <= 3u) {
        return NL_PHY_ERR_MODE;
    }
    ...
    return nl_zc_gen_zc(u, v, alpha, M_rs, seq);
}

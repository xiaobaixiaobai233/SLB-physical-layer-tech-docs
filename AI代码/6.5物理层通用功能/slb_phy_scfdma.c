/**
 * @file slb_phy_scfdma.c
 * @brief 6.5.8 SC-FDMA DFT 预编码
 */
#include "slb_phy_scfdma.h"

#include <math.h>

static void slb_dft_segment(const slb_cplx_t *x, slb_cplx_t *y, uint16_t M)
{
    uint16_t k;
    uint16_t i;
                       ...
                       uint16_t M_sc_total)
{
    ...
    L = M_symb / M_sc_total;
    for (l = 0u; l < L; l++) {
        slb_dft_segment(x + l * M_sc_total, y + l * M_sc_total, M_sc_total);
    }
    return SLB_PHY_OK;
}

...
G2 & \texttt{frameCarrier} & 符号类型、$N_{\mathrm{TTI}}$、$N_{\mathrm{CH}}$/$L_{\mathrm{CH}}$、中心频点、传输模式、链路布局、域类型 \\
...
\noindent\textbf{读}：6.2.2 帧网格、OFDM、$N_{\mathrm{CP}}$ 派生、多载波布局、TTI/链路时分校验。\\
\textbf{注}：$N_{\mathrm{CH}}$/$L_{\mathrm{CH}}$ 档位表属 L0 \texttt{nChLchPairTable}，本结构只存当前会话选中值；\texttt{linkSymbolLayout} 须满足 G 开头与末切换间隔等强制结构。
...
\textbf{对象} & \textbf{去向} \\
\midrule
L0 常量（$f_s$、161、光速…）及查表 & \texttt{slb\_phy\_const} / L0 表，不进 Session \\
GCI 格式 0--2 当次资源 & 调用 \texttt{slbPosApplyGciResource} / L3 当前资源槽 \\
GCI-4 激活位 & 事件入参，非长期配置 \\
当次功控 $M$（子载波数） & 发送调用参数（L3） \\
COT 起止帧 / TTI 序号 / RE $(k,l)_p$ & 细模块 \texttt{tti.*}/\texttt{re.*}（偏 L3），非 Session 常驻 \\
直通先发/后发角色 & \texttt{link.passthrough} 当次参数 \\
模块内 L2（如 ZC 的 $u,v$ 细节） & 各细模块（如 \texttt{zc.generate}）私有配置 \\
\bottomrule
\end{longtable}

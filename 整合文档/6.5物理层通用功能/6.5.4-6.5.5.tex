...
\textbf{接口} & \textbf{输入} & \textbf{输出} & \textbf{说明} \\
\midrule
\texttt{slb\_meas\_pms\_csi} & 频域 IQ[] & IQ[linear], RPL & 6.5.5.6 \\
\texttt{slb\_meas\_pms\_timediff} & $t_1\ldots t_6$ & $T$ 或 $T_1,T_2$ & 6.5.5.7 \\
\texttt{slb\_meas\_pms\_aoa} & 阵列快照 & 水平角、垂直角 & 6.5.5.8 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
  \item 估计子载波 $k$ 信道系数 IQ[linear]；
  \item 配置 RPL[dBm] 标尺；
  \item 按式计算 IQ[dBm] 供 6.6 解码。
\end{enumerate}

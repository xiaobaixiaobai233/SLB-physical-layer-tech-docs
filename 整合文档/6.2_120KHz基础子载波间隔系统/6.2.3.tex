...
\begin{longtable}{L{2.2cm}L{4.2cm}L{6.1cm}}
\caption{文档范围与模块划分} \\
\toprule
...
\caption{FTS/STS 与 SAB/RAB 符号布局} \\
\toprule
...
\subsection{Zadoff-Chu（ZC）专用序列的含义}

6.2.3 节将物理层辅助波形所用的序列分为\textbf{两大类}：ZC 专用序列（FTS/STS/PMS）与伪随机 QPSK 序列 $r_{n,l}(k)$（SRS/CSI-RS/DMRS/PAS 等）。``ZC 专用''并非指须调用 6.5.9 节通用 ZC 框架，而是指\textbf{FTS/STS/PMS 在 6.2.3 中直接以 Zadoff-Chu 二次相位闭式公式定义}，与 Gold 序列 $c(i)$ 无关。

\textbf{Zadoff-Chu 标准形式}（序列长度 $N$，根索引 $u$）：
\begin{equation}
x_u(n) = e^{-j\pi u\, n(n+1)/N}, \quad 0 \leq n < N
\end{equation}

\textbf{6.2.3 中的 ZC 专用实例}：
\begin{longtable}{llll}
\caption{6.2.3 ZC 专用序列与标准公式对应} \\
\toprule
\textbf{信号} & \textbf{公式} & \textbf{$N$} & \textbf{$u$ 含义} \\
\midrule
\endfirsthead
...
\bottomrule
\endfoot
FTS & $d_{\mathrm{FTS}}(n)=e^{-j\pi u n(n+1)/161}$ & 161 & 场景标识（1/160/2/159） \\
STS & $d_{\mathrm{STS}}(n)=e^{-j\pi u (n/8)((n/8)+1)/21}$（梳状） & 21 & 节点 ID 低 4 位 + 1 \\
PMS & $d(n)=e^{-j\pi u n(n+1)/161}$ & 161 & 测距端口配置（3\textasciitilde 20） \\
\end{longtable}

\textbf{ZC 序列的主要性质}：恒包络（低 PAPR）、理想周期自相关（同步/测距相关峰尖锐），故用于同步与专用测距；$u$ 不同则序列正交性良好，可用于区分场景或节点。

\textbf{与 6.5.9 通用 ZC 框架的区别}：6.5.9 定义 $r_{u,v}^{(\alpha,\delta)}(n)=e^{j\alpha n}\bar r_{u,v}(n)$，含基序列组 $u\in\{0,\ldots,29\}$、组内序号 $v$、循环移位 $\alpha$ 及可变长度 $M_{\mathrm{SC}}^{\mathrm{RS}}$。6.2.3 的 FTS/STS/PMS \textbf{不经过}该参数化流程，$u$ 的含义亦不同（场景/节点标识，而非 6.5.9 的组号）。实现 6.2.3 时\textbf{按本节闭式公式直接生成}即可。

\textbf{与伪随机 QPSK 的区别}：$r_{n,l}(k)$ 源自 31 阶 Gold 序列 $c(i)$（6.5.2/6.5.3），适合大量、灵活的参考导频；ZC 专用序列\textbf{不经过} $c(i)$ 与 $r_{n,l}(k)$ 映射。

\subsection{31 阶 Gold 伪随机序列 $c(i)$（6.5.2）}

6.2.3 节中多数参考信号与相位调整信号所用的伪随机 QPSK 序列 $r_{n,l}(k)$，其比特源为 6.5.2 节定义的 31 阶 Gold 伪随机序列 $c(i)$。$c(i)$ 由两条 31 阶 m 序列 $x_1(i)$、$x_2(i)$ 模 2 相加得到（长度为 $M_{\mathrm{PN}}$，$i=0,1,\ldots,M_{\mathrm{PN}}-1$）：
\begin{equation}
...
\textbf{$x_1$ 初始化（固定）}：
\begin{equation}
x_1(0)=1,\quad x_1(i)=0,\; i=1,2,\ldots,30
\end{equation}
因 $x_1$ 的初态在所有应用场景下均相同，\textbf{整条 $x_1$ 序列与 $N_{\mathrm{ID}}$、$n$、$l$ 等参数无关}，对任意设备、任意时频资源均一致。

\textbf{$x_2$ 初始化（随场景变化）}：由 $c_{\mathrm{init}}$ 决定，$c_{\mathrm{init}}$ 的 31 个比特即为 $x_2(0),\ldots,x_2(30)$ 的初态：
\begin{equation}
c_{\mathrm{init}} = \sum_{i=0}^{30} x_2(i)\cdot 2^i
\end{equation}
$c_{\mathrm{init}}$ 的具体取值依 $c(i)$ 的应用场景而定。对 6.2.3 参考信号所用的 $r_{n,l}(k)$，$c_{\mathrm{init}}$ 由 6.5.3 节公式给出（见下节），随无线帧号 $n$、符号号 $l$、$N_{\mathrm{ID}}$、$N_{\mathrm{CP}}$ 变化；因此 \textbf{$x_2$ 须按当前 RE 上下文实时递推}，不能全局预置。

\textbf{生成顺序}：实现时须先按 6.5.2 递推 $x_1$、$x_2$ 至下标 $i+1600$，再按上式得 $c(i)$，最后经 6.5.3 映射为 QPSK 的 $r_{n,l}(k)$。

\subsubsection{代码实现提示（$x_1$ 预计算）}

\begin{itemize}[nosep]
  \item $c(i)$ 依赖 $x_1(i+1600)$ 与 $x_2(i+1600)$。对 6.2.3 中 $r_{n,l}(k)$（$m=0,\ldots,160$），需 $c(0)\sim c(321)$，即 $x_1$、$x_2$ 至少递推至下标 $1600+321=1921$。
  \item \textbf{$x_1$ 可离线预生成}：$x_1$ 初态固定，递推多项式不变，整条序列确定。可在编译期或模块初始化时一次性生成 $x_1(0)\sim x_1(N_{\max})$（建议 $N_{\max}\geq 1921$，若需支持更长 $M_{\mathrm{PN}}$ 则取更大上界），存入只读查找表 \texttt{x1\_lut[]}。
  \item \textbf{运行时仅算 $x_2$}：按当前 $c_{\mathrm{init}}$ 初始化 $x_2$ 的 31 级 LFSR，递推至 $x_2(i+1600)$，再与 \texttt{x1\_lut[i+1600]} 模 2 相加得 $c(i)$。同一 $(n,l,N_{\mathrm{ID}},N_{\mathrm{CP}})$ 下 $c_{\mathrm{init}}$ 不变，可对 $x_2$ 段做缓存复用。
  \item \textbf{预期收益}：避免每次生成 $r_{n,l}(k)$ 时重复递推 $x_1$ 的前 1600+ 项；在 SRS/CSI-RS/DMRS 等高频调用路径上可显著降低 LFSR 运算量。
  \item \textbf{注意}：安全位置测量（6.2.3.9.2）对 $c(i)$ 做 KDF 异或加扰时，$c(i)$ 仍须先按 6.5.2/6.5.3 正确生成，再进入加扰流程；$x_1$ 预计算不改变 $c(i)$ 数值，仅优化生成路径。
\end{itemize}
...
\end{equation}
其中 $c(i)$ 为上节（6.5.2）定义的 31 阶 Gold 伪随机序列；$x_2$ 的初态由下列 $c_{\mathrm{init}}$ 的 31 比特给出（6.5.3）：
\begin{equation}
...
\subsection{序列预计算与查表策略（代码实现）}

将 6.2.3 各类物理层信号由公式转为代码时，可按\textbf{整表查、部分预计算、必须实时计算}三类划分，以降低运行时 LFSR/指数运算开销。

\begin{longtable}{L{2.4cm}L{3.2cm}L{2.2cm}L{5.2cm}}
\caption{6.2.3 物理层信号预计算策略总览} \\
\toprule
\textbf{信号} & \textbf{序列来源} & \textbf{策略} & \textbf{实现要点} \\
\midrule
\endfirsthead
\multicolumn{4}{c}{\small（续表）} \\
\toprule
...
\bottomrule
\endfoot
FTS & ZC，$u\in\{1,2,159,160\}$ & 整表查 & \texttt{fts\_lut[u][161]}，4 张表 \\
STS & ZC 梳状，$u=1{\sim}16$ & 整表查 & \texttt{sts\_lut[u][]}，含 $2\sqrt{2}$ 缩放 \\
PMS & ZC，$u=3{\sim}20$ & 整表查 & \texttt{pms\_lut[u][161]}；多载波对单载波 $a$ 预算后按表~\ref{tab:623-pms-concat} 拼接 \\
SRS/CSI-RS & Gold $\rightarrow r_{n,l}$ & 部分预计算 & $x_1$ 必预计算 \texttt{x1\_lut[]}；$x_2$ 按 $c_{\mathrm{init}}$ 递推或缓存 \\
DMRS-C/D & Gold $\rightarrow r_{n,l}$ & 部分预计算 & 同上；同一 $(n,l,N_{\mathrm{ID}},N_{\mathrm{CP}})$ 可缓存 $r_{n,l}$ \\
PAS & Gold $\rightarrow r_{n,l}$ & 部分预计算 & 仅 $k_0,k_1$ 两点，复用 QPSK 生成 \\
安全 PMS & KDF $\oplus$ $c(i)$ & 部分 + 实时 & $x_1$/基础 $c(i)$ 可优化；KDF 异或后波形必须实时算 \\
安全感知 & 置乱 $\rho_i$ & 实时 & 随保密参数/事件变化，不可整表 \\
RE 映射位置 & 调度/固定规则 & 规则表 & \#6/\#0 等固定位置查规则；动态调度即时算 \\
\end{longtable}

\textbf{优先实现顺序}：
\begin{enumerate}[nosep]
  \item \textbf{FTS/STS/PMS 整表}：$u$ 取值有限、公式固定，启动时离线生成，运行时 $O(1)$ 查表；收益最大、实现最简单。
  \item \textbf{Gold 路径 $x_1$ 预计算}：$x_1$ 初态固定（见 6.5.2），整条序列与场景无关；建议 \texttt{x1\_lut[0..N\_max]}，$N_{\max}\geq 1921$。
  \item \textbf{$x_2$ 递推或缓存}：$c_{\mathrm{init}}$ 随 $n,l,N_{\mathrm{ID}},N_{\mathrm{CP}}$ 变；运行时 LFSR 递推 $x_2$，或对常用组合缓存 $c(i)$/$r_{n,l}(k)$。
  \item \textbf{（可选）全 $r_{n,l}$ 表}：组合数 $\approx 8\times14\times256\times4 \sim 10^5$，全表约百 MB 级；内存允许时可整表，否则 $x_1$ 查表 + $x_2$ 递推更常见。
  \item \textbf{安全类不整表}：安全 PMS/感知最终加扰序列依赖 \texttt{rand}/KDF/置乱数，须实时生成以保证安全。
\end{enumerate}

\textbf{禁止事项}：预计算/查表仅优化\textbf{生成路径}，不得硬编码与标准不符的固定导频图案；查表内容须与 6.2.3/6.5.2/6.5.3 公式数值一致。
...
单载波 PMS：$d(n)$ 为 ZC 型序列，$u{\in}[3,20]$。多载波时域拼接及安全加扰详见 \S6.2.3.9；完整拼接配置见表~\ref{tab:623-pms-concat}。

%----------------------------------------------------------------------
\section{本节核心详解}
%----------------------------------------------------------------------

\subsection{6.2.3.1 同步信号 FTS/STS}

\textbf{功能}：为接收端提供定时/频偏同步与节点识别；构成 SAB/RAB 同步核心（与 6.2.4.2 衔接）。

\textbf{序列体系}：ZC 专用序列（闭式二次相位），\textbf{不经过} 6.5.9 通用 ZC 框架，\textbf{不经过} Gold $c(i)$/$r_{n,l}(k)$。

\subsubsection{第一训练信号 FTS}

\begin{equation}
d_{\mathrm{FTS}}(n) = e^{-j\pi u\, n (n+1) / 161},\quad n{\neq}80;\quad d_{\mathrm{FTS}}(80)=0
\end{equation}
映射：$a_{k,l}=d_{\mathrm{FTS}}(k)$，$k=0,\ldots,160$。

\textbf{双符号结构}：连续 2 符号映射相同 FTS；首符号 CP 加倍（抗多径），次符号\textbf{无 CP}。

\textbf{标识 $u$}：1（通信域 SAB，类型一 A/B/四）；160（通信域 SAB，类型二/三）；2（RAB 接入）；159（直通 SAB）。

\subsubsection{第二训练信号 STS}

\begin{equation}
d_{\mathrm{STS}}(n) = e^{-j\pi u\, (n/8)\, \bigl((n/8)+1\bigr) / 21}
\end{equation}
仅 $n=0,8,16,\ldots,160$ 且 $n{\neq}80$ 有效。映射：$a_{k,l}=2\sqrt{2}\, d_{\mathrm{STS}}(k)$（功率与 FTS 匹配）。

\textbf{单符号}映射；$u=1,\ldots,16$ = G 节点 ID 或直通先发节点 ID 低 4 位 $+1$，用于细同步与节点识别。

\subsubsection{SAB/RAB 符号布局}

\begin{longtable}{llll}
\caption{FTS/STS 在 SAB/RAB 中的符号位置} \\
\toprule
\textbf{场景} & \textbf{\#0} & \textbf{\#1,\#2} & \textbf{FTS $u$} \\
\midrule
\endfirsthead
\multicolumn{4}{c}{\small（续表）} \\
\toprule
...
\bottomrule
\endfoot
通信域 SAB & STS & FTS & 1 或 160 \\
直通 SAB & STS & FTS & 159 \\
RAB 接入 & STS & FTS & 2 \\
无 SAB 载波 & STS & --- & --- \\
\end{longtable}

\textbf{流程上下文}：T 盲检 STS/FTS 相关峰（6.2.7）$\rightarrow$ 粗/精同步 $\rightarrow$ 识别场景与节点 $\rightarrow$ 读 SAB 同步信息。

\textbf{实现提示}：FTS 4 张表（$u{=}1/2/159/160$）、STS 16 张表可整表预计算；运行时 $O(1)$ 查表。
...
\textbf{第二种模式（时域多段拼接）}：第一种模式单载波信号 IFFT 得时域段 $a$；$a^*$ 为共轭，$b$ 为对称，$b^*$ 为 $b$ 共轭；按表~\ref{tab:623-pms-concat} 拼接后按 6.2.2.1 加 CP：

\begin{longtable}{ll}
\caption{不同带宽下多基础载波位置测量信号配置（见 6.2.3.9.2）\label{tab:623-pms-concat}} \\
\toprule
...
\textbf{置乱数同步}（表~\ref{tab:623-scramble-sync}）：
\begin{longtable}{ll}
\caption{置乱数同步方式（见 6.2.3.10.2）\label{tab:623-scramble-sync}} \\
\toprule
\textbf{方式} & \textbf{说明} \\
\midrule
\endfirsthead
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{安全感知}：每过程 $S$ 个事件、间隔 $T$；置乱数须表~\ref{tab:623-scramble-sync} 方式一或二同步。
\end{enumerate}
...
\textbf{Step 2}：若使用 $r_{n,l}(k)$，先按 6.5.2 生成 Gold 序列 $c(i)$（$x_1$ 可查表，$x_2$ 由 $c_{\mathrm{init}}$ 递推），再按 6.5.3 映射为 QPSK。
...
\textbf{Step 3}：安全模式一：KDF 加扰 $c(i)$ $\rightarrow$ $D(j)$ $\rightarrow$ QPSK；安全模式二：单载波 IFFT 得 $a$，按表~\ref{tab:623-pms-concat} 拼接并加 CP。

\subsection{感知测量信号发送（6.2.3.10）}

\textbf{Step 1}：解析感知资源指示，选择 CSI-RS/SRS/PMS 作为感知参考。

\textbf{Step 2}：安全感知：配置 $N_t\geq 2$ 天线；按事件分 $P$ 时段生成 $X(k,n)$ 矩阵；同步置乱数（表~\ref{tab:623-scramble-sync}）。
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{【序列路径】}：ZC 专用序列（FTS/STS/PMS）按 6.2.3 闭式公式生成，不经 6.5.9；参考信号 $r_{n,l}(k)$ 须经 6.5.2/6.5.3。
  \item \textbf{【预计算边界】}：查表仅优化生成路径，不得硬编码与标准不符的导频图案；安全 PMS/感知须实时计算。
  \item \textbf{【安全 PMS】}：KDF counter 序列长度须与基础载波数严格匹配。
  \item \textbf{【安全感知】}：$N_t \geq 2$；置乱数须表~\ref{tab:623-scramble-sync} 方式一或二同步。
  \item \textbf{【无状态机】}：信号生成与映射用配置/调度指示/标志位显式条件推进，不维护 FSM 跳转表，避免异常路径锁死。
\end{enumerate}
